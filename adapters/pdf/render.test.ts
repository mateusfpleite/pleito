...
/**
 * Testes DETERMINÍSTICOS da render de PDF. NÃO testam o binário chromium
 * (RESÍDUO de runtime — só existe no worker container; valida em
 * deploy/E2E, @superpowers:testing-anti-patterns). Aqui provamos O QUE É
 * NOSSO: (1) o HTML montado contém os campos certos, (2) o escape
 * anti-XSS na conversão markdown→HTML, (3) o contrato Buffer via engine
 * FAKE determinístico.
 */

/** Engine fake: devolve um Buffer `%PDF` determinístico e registra o
 * HTML que recebeu (p/ assertir a montagem). NÃO renderiza nada real. */
function fakeEngine(): PdfEngine & { htmlVisto: string[] } {
  ...
  return {
    ...
    async htmlParaPdf(html: string): Promise<Buffer> {
      htmlVisto.push(html);
      // Buffer determinístico que COMEÇA com a assinatura de PDF.
...
describe('markdownParaHtmlSeguro (minimal and safe conversion)', () => {
  it('does NOT let user markup pass through (XSS blocked)', () => {
    ...
    // Invariante de segurança: nenhuma TAG injetada pelo usuário. O `<`/`>`
    // do conteúdo vira entidade — `onerror=`/`onclick=` só sobrevivem como
    // TEXTO inerte dentro de `&lt;...&gt;` (sem tag real, não executam).
    ...
    // Estrutura legítima (gerada por NÓS) preservada.
...
describe('renderRelatorioPdf / renderOficioPdf (contrato Buffer via engine fake)', () => {
  it('renderRelatorioPdf returns a Buffer starting with %PDF', async () => {
    ...
    // O engine recebeu o HTML do relatório (projeção do JSON).
